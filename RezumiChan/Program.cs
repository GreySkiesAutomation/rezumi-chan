...
namespace RezumiChanCLI
{
    public class Program
    {
        ...
        const string modelToUseIntense = "gpt-5-mini-2025-08-07";
        ...
        public static async Task<List<Skill>> GetRelevantSkills(Resume resume, JobPost job)
        {
            ...
            var prompt =
                ...
                $"Return ALL skill IDs reordered by relevance to the job. " +
            ...
            var parsed =
                JsonConvert.DeserializeObject<OrderedSkillIdsResponse>(rawJson)
            ...
            foreach (var id in parsed.OrderedSkillIds)
            {
                ...
                if (!grouped.ContainsKey(entry.Category))
                {
                    grouped[entry.Category] = new List<string>();
                }

                grouped[entry.Category].Add(entry.Skill);
            }

            // --- Convert back to List<Skill> ---
        ...
        public static async Task<List<Skill>> GetRelevantSkillsOld(Resume resume, JobPost job)
        {
            var apiKey = LoadApiKey();
            var endpoint = "https://api.openai.com/v1/chat/completions";
            ...
            IProgress<PipelineProgress>? progress = null)
        {
            ...
            var tcSelected = await GetBulletPointsFromBank(tcBank, job, 4, false);
            var waveSelected = await GetBulletPointsFromBank(waveBank, job, 4, false);
